...
**Revisão Responsiva Plena (22/02):** Correções na Landing Page para restaurar o botão "Entrar" escondido em visualizações de dispositivos móveis, e quebra de texto customizada nos CTAs principais que "comiam letras" no Mobile. Adição também do componente nav-*Sheet* (Menu Hambúrguer animado) no `app/dashboard/layout.tsx` para possibilitar a transição entre telas do sistema nos celulares sem ocultar a navegação.
**Correção de Build/Vercel (22/02):** Resolvido o erro de build que travava os deploys recentes na Vercel (Next.js `module-not-found`). O bug ocorreu devido à deleção recursiva da pasta inteira `app/dashboard/nfe` durante a otimização de redundância, que acabou apagando acidentalmente componentes internos globais essenciais das tabelas (`nfe-actions.tsx`) e a página de visualização estendida (`[id]/page.tsx`). Os arquivos de estado interno foram restaurados através do controle de versão (Git) e mantiveram a exclusão de redundância correta, focado apenas no `nfe/page.tsx`.
**Aprimoramento de UI Premium (22/02):** Modificação global na barra superior (*Header*) do Dashboard. O fundo, antes claro/monótono, foi reestilizado com a cor preestabelecida da marca (Azul Primary). Substituição aplicada incorporando o logotipo branco translúcido (`logo_tela_login.png`) em contraponto, acompanhado do refinamento dos ícones flutuantes — O botão menu de dispositivos móveis, o sino de Notificações nativas, e as bordas de avatar/iniciais agora possuem efeito iluminado translúcido (`text-white hover:bg-white/20`) garantindo total harmonia e requinte sobre a barra primária.
**Download em Lote — Plano Pró (22/02):** Implementado módulo completo de download em lote de arquivos fiscais exclusivo para usuários Pro. API Route server-side (`app/api/download-lote/route.ts`) gera um ZIP via `JSZip` com 3 modalidades selecionáveis: XMLs (até 500 notas, instantâneo), DANFEs em PDF (até 100 notas, via cache MeuDanfe API + bucket Supabase Storage), ou ambos simultaneamente. O frontend recebeu um dropdown estilizado ("Baixar em Lote") na toolbar do Dashboard com feedback visual de progresso (azul info → verde sucesso) e notificação proativa de limite atingido. Plan gating server-side rejeita acessos Starter com HTTP 403. Filtros de período aplicados automaticamente.
