...
export async function middleware(request: NextRequest) {
    ...
    // Redirecionar usuário autenticado da raiz/login
    ...
    if (user && pathname.startsWith('/dashboard')) {
        try {
            // Criar cliente admin para consulta que bypassa RLS
            ...
            // Master admins sempre têm acesso ao dashboard
            const email = user.email?.toLowerCase() ?? ''
            if (MASTER_ADMIN_EMAILS.includes(email)) {
            ...
            // O frontend pode verificar novamente
        }
    }
